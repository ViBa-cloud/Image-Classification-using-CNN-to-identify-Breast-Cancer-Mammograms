...
TRAIN_IMAGES_PATH = "/scratch/stats_dept_root/stats_dept1/nawatsw/eecs545/original_data_png/train_images"
...
class BreastCancerModel(torch.nn.Module):
    def __init__(self, model_type=Config.MODEL_TYPE, dropout=0.):
        super().__init__()
...
def evaluate_model(model: BreastCancerModel, ds, max_batches=PREDICT_MAX_BATCHES, shuffle=False, config=Config):
    ...
    with torch.no_grad():
        ...
        return np.mean(cancer_losses), (pf1, thres), pred, np.mean(losses), precision, recall
...
def train_model(ds_train, ds_eval, logger, name, config=Config, do_save_model=True):
    ...
    for epoch in tqdm(range(config.EPOCHS), desc='Epoch'):
        ...
        if ds_eval is not None and MAX_EVAL_BATCHES > 0:
            cancer_loss, (f1, thres), _, loss, precision, recall = evaluate_model(
            ...
            print({'eval_cancer_loss': cancer_loss, 'eval_f1': f1, 'max_eval_f1': best_eval_score,
                   'eval_f1_thres': thres, 'eval_loss': loss, 
...
if __name__ == "__main__":
    ...
    for fold in FOLDS:
        ...
        train_model(ds_train, ds_eval, None, MODEL_NAME)